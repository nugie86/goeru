...
def load_data():
    ...
    item_train = genfromtxt('./data/data_train_item.csv', delimiter=',')
    user_train = genfromtxt('./data/data_train_user.csv', delimiter=',')
    y_train    = genfromtxt('./data/y_train.csv', delimiter=',')
    with open('./data/item_header.txt', newline='') as f:    #csv reader handles quoted strings better
        item_features = list(csv.reader(f))[0]
    with open('./data/user_header.txt', newline='') as f:
...
#    with open('./data/movies.csv', newline='') as csvfile:
    with open('./data/item_dicts.csv', newline='') as csvfile:
...
def print_pred_movies(y_p, item, movie_dict, maxcount=10):
    ...
    count = 0
    disp = [["y_p", "teacher id", "rating ave", "name", "fields"]]
    ...
    for i in range(0, y_p.shape[0]):
        ...
        disp.append([np.around(y_p[i, 0], 1), item[i, 0].astype(int), np.around(item[i, 1].astype(float), 1),
